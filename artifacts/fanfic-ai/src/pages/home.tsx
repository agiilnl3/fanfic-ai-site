      ...
      <section className="relative pt-24 pb-32 overflow-hidden flex flex-col items-center justify-center min-h-[70vh]">
        ...
        <div className="container mx-auto px-4 relative z-10 text-center max-w-4xl">
          <Badge className="mb-6 bg-primary/10 text-primary border-primary/20 backdrop-blur-sm hover:bg-primary/20">
            <Sparkles className="w-3 h-3 mr-2" /> AI-Powered Storytelling
          </Badge>
          <h1 className="font-serif text-5xl md:text-7xl font-bold tracking-tight mb-6 glow-text text-foreground">
            Conjure Worlds with <br/><span className="text-primary italic">Ink & Algorithm</span>
          </h1>
          <p className="text-lg md:text-xl text-muted-foreground mb-10 max-w-2xl mx-auto leading-relaxed">
            Step into the creative studio where your prompts transform into richly illustrated, full-length tales. Every generation is a uniquely crafted manuscript.
          </p>
        ...
        <div className="container mx-auto px-4 mt-24">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 max-w-4xl mx-auto">
            <StatBox label="Stories Conjured" value={statsLoading ? "-" : stats?.totalStories} />
            <StatBox label="Published Works" value={statsLoading ? "-" : stats?.publishedStories} />
            <StatBox label="Illustrations" value={statsLoading ? "-" : stats?.totalIllustrations} />
            <StatBox label="Active Genres" value={statsLoading ? "-" : stats?.genreBreakdown?.length || 0} />
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-secondary/30 border-t border-border/50">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-end mb-12">
            <div>
              <h2 className="font-serif text-3xl md:text-4xl font-bold text-foreground mb-3">Recent Masterpieces</h2>
              <p className="text-muted-foreground">The latest published works from our authors.</p>
            </div>
            <Link href="/feed">
